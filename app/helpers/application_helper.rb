module ApplicationHelper
  ...
  def is_active(tag=[], method=[])
    return "active" if method.empty? && tag.include?(controller_name)
    return "active" if method.include?(action_name) && tag.include?(controller_name)
  end
end
